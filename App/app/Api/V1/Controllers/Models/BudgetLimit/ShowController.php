...
class ShowController extends Controller
{
    ...
    public function index(Request $request, Budget $budget): JsonResponse
    {
        ...
        $paginator->setPath(ShowController . phproute('api.v1.budgets.limits.index', [$budget->id]) .
            $this->buildParams());
    ...
    public function indexAll(SameDateRequest $request): JsonResponse
    {
        ...
        $budgetLimits = $collection->slice(($this->parameters->get('page') - 1) * $pageSize, $pageSize);
        $paginator    = new LengthAwarePaginator($budgetLimits, $count, $pageSize, $this->parameters->get('page'));
        $paginator->setPath(ShowController . phproute('api.v1.budget-limits.index') . $this->buildParams());
